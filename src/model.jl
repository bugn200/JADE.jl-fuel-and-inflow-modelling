...
function JADEsddp(d::JADEData, optimizer=nothing)
    ...
    sddpm = SDDP.PolicyGraph(
        ...
        if stage < number_of_wks || !d.rundata.use_terminal_mwvs
            ...
            SDDP.@stageobjective(md, immediate_cost / scale_obj+ 0.01*fuel_stockpile.out + 250000 * step[1] + 300000 * step[2] + 350000 * step[3])
        else
            ...
            SDDP.@stageobjective(md, immediate_cost / scale_obj + terminalcost + 0.01*fuel_stockpile.out + 250000 * step[1] + 300000 * step[2] + 350000 * step[3])
        end
    end
