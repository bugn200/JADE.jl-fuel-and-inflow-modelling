...
function JADEsddp(d::JADEData, optimizer=nothing)
    ...
    sddpm = SDDP.PolicyGraph(
        ...
        if stage < number_of_wks || !d.rundata.use_terminal_mwvs
            ...
            SDDP.@stageobjective(md, immediate_cost / scale_obj+ fuel_stockpile.out + 500 * step[1] + 750 * step[2] + 1000 * step[3])
        else
            ...
            SDDP.@stageobjective(md, immediate_cost / scale_obj + terminalcost + fuel_stockpile.out*0.5 + 2500 * step[1] + 3500 * step[2] + 8500 * step[3])
        end
    end
