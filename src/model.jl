...
function JADEsddp(d::JADEData, optimizer=nothing)
    ...
    sddpm = SDDP.PolicyGraph(
        ...
        if stage < number_of_wks || !d.rundata.use_terminal_mwvs
            ...
            SDDP.@stageobjective(md, immediate_cost / scale_obj+ 1.5*fuel_stockpile.out + 250 * step[1] + 300 * step[2] + 350 * step[3])
        else
            ...
            SDDP.@stageobjective(md, immediate_cost / scale_obj + terminalcost + fuel_stockpile.out*1.5 + 250 * step[1] + 300 * step[2] + 350 * step[3])
        end
    end
